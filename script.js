...
async function updateRadar(lat, lng) {
    try {
        ...
        document.getElementById('nearGrid').innerHTML = html || "No landmarks nearby in 10KM.";
    } catch (e) {
...
function initiateSystem() {
    ...
    document.getElementById('mainBtn').innerHTML = '<i class="fas fa-sync fa-spin"></i> RADAR ACTIVE';
    document.getElementById('mainBtn').style.background = "#000";
    ...
    navigator.geolocation.watchPosition(async pos => {
        ...
        if (address && address !== lastLoggedAddr) {
            ...
            if (log.innerText.includes("Timeline starts")) log.innerHTML = "";
            
            const entry = document.createElement('div');
            entry.className = 'log-entry';
            entry.innerHTML = `<b>${time}</b>: ${address}`;
            log.prepend(entry);
        }

        const msg = `Arpalink Live: ${address} | Maps: https://www.google.com/maps?q=${latitude},${longitude}`;
